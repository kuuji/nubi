...
def main() -> int:
    ...
    try:
        ...
        if pr_url:
            logger.info("Posting pipeline summary comment to %s", pr_url)
            summary_posted = post_pipeline_summary(
                pr_url=pr_url,
                repo=repo,
                branch=task_branch,
                token=token,
            ...
            if summary_posted:
                logger.info("Pipeline summary posted successfully")
            else:
                logger.warning("Failed to post pipeline summary comment")
